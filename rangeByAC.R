...
rangeByAC = function(envAreas, movRes, sps_range=NULL, iter=100){
  
  ## atualizando distribuicoes atraves do tempo ##
  # envAreas: suitability gridfile for sps range
  # movRes: raster layer with data for resistance to species movement throughout the landscape (equivalent to migration)
  # sps_range: raster layer of sps range (it could be computed before)
  ...
  if (!is.null(sps_range)){
    ##usando sps range fornecida pelo usuario
    spRange_i = raster::merge(sps_range, spRange)
  } else {
    ##semente (i.e. ponto de origem para o crescimento do range da Sp)
  ...
  for (i in 1:iter){
    ...
    borderOcc = borderOcc*borderMig #occupancy of borders accounting for environmental niche AND environmental resistance for species movement
  ...
  spRange = spRange_i
  spRange[values(spRange) == 0] = NA
  
  maskHelper = spSuit
  maskHelper[values(maskHelper) > 0] = 0
  
  #spRange = spRange*maskHelper
  spRange = raster::merge(spRange, maskHelper)
